...
**Compose generation** (`internal/deployment/`): on every `Start`, a Go template (`docker-compose.yaml.tmpl`, embedded via `//go:embed`) is rendered with the config and written to `docker.compose_output_path`. Edit the template to change the Compose structure; no Go code changes required.

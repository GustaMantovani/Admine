...
class RedisPubSubServiceProvider(PubSubService):
    def __init__(self, logging: Logger, host: str, port: int, subscribed_channels: list[str], producer_channels: list[str]):
        ...
        self.__pubsub = self._client.pubsub()
        self.__logger.info(f"Redis client initialized at {host}:{port}")

    def send_message(self, message: AdmineMessage):
        self.__logger.debug(f"Sending message to channels: {', '.join(self.get_producer_channels())}")

    def listen_message(self):
        self.__logger.debug(f"Listening to channels: {', '.join(self.get_subscribed_channels())}")

...
class CommandHandle:
        ...
        if command in self.__HANDLES:
            ...
            if hasattr(handler, "admin_only") and handler.admin_only:
                if not administrators or not user_id or user_id not in administrators:
                    logger.warning(f"User {user_id} attempted to use admin command: {command} without permission")
                    return "Unauthorized command usage"
                logger.info(f"Admin command {command} authorized for user {user_id}")
    ...
    async def __command(self, args: List[str]):
        logger.debug(f"Execute a command in Minecraft with args: {args}")
        try:
    ...
    async def __info(self, args: List[str]):
        logger.debug(f"Getting info off the server with args: {args}")
        try:
    ...
    async def __status(self, args: List[str]):
        logger.debug(f"Getting status off the server with args: {args}")
        try:
    ...
    async def __server_ips(self, args: List[str]):
        logger.debug(f"Getting server ip the server with args: {args}")
        try:
    ...
    async def __turn_admin(self, args: List[str]):
        logger.debug(f"Adding administrator with args: {args}")
    ...
    async def __add_channel(self, args: List[str]):
        logger.debug(f"Adding channel ID with args: {args}")
        if not args or not args[0]:
    ...
    async def __remove_channel(self, args: List[str]):
        logger.debug(f"Removing channel ID with args: {args}")
        if not args or not args[0]:

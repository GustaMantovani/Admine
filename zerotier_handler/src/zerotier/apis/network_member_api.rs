...
pub async fn delete_network_member(configuration: &configuration::Configuration, network_id: &str, member_id: &str) -> Result<(), Error<DeleteNetworkMemberError>> {
    ...
    let local_var_uri_str = format!("{}/network/{networkID}/member/{memberID}", local_var_configuration.base_path, networkID=crate::zerotier::apis::urlencode(network_id), memberID=crate::zerotier::apis::urlencode(member_id));
